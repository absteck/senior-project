...
*I am using this file to test whether the change in the shape of my confidence intervals is related to changing the way that I coded the sentence.quantiles so that people fall into the category of the highest threshold they pass, instead of the lowest.*
...
### Read data 
```{r, message=FALSE}
#PROBLEM: have gotten a "fatal error" message when I run this block, but when I run each line individually it seems fine  
library(R.utils)
library(tidyverse)
...
outcome <- c('SENTENCE') #1 continuous response
...
table(df$sentence.quartiles) #NOTE: everyone is in "above 0.03" 
...
#QUESTION: What is formula.full.race used for?  
...
dim(X.base) #442187 rows, 2 cols. should have same nrows as df
#1st column is column of 1s, 2nd column is race data 
X.full <- sparse.model.matrix(formula.full, df)
#dim(X.full) #442187 x 57. There are 57 columns (as opposed to just 26) because each primary offense level gets its own column. 
...
if (!file.exists('mods_base_correct_blackwhite_sentencing_wrong.rds')){
  mods.base.correct <- lapply(
    ...
    function(thresh_index){
      ...
      vcovCL <- vcovCL.speedglm(mod,
                                ...
                                X.base,
                                cluster = df$PRIM_OFFENSE #each observation is a cluster (ie no clustering)
                                )
      std_err <- sqrt(diag(vcovCL)) 
      return(list(thresh = sentence.quartiles[thresh_index], mod = mod, vcovCL = vcovCL, std_err = std_err))
    })
  saveRDS(mods.base.correct, 'mods_base_correct_blackwhite_sentencing_wrong.rds')
} else {
  mods.base.correct <- readRDS('mods_base_correct_blackwhite_sentencing_wrong.rds')
}
...
(p.white.q2 <- log.odds.to.prob(mods.base.correct[[2]]$mod$coefficients[1], mods.base.correct[[1]]$mod$coefficients[2], 0))
#probability that a black person receives an above-median sentence 
(p.black.q2 <- log.odds.to.prob(mods.base.correct[[2]]$mod$coefficients[1], mods.base.correct[[1]]$mod$coefficients[2], 1))

#probability that a white person receives an above-first-quartile sentence 
(p.white.q1 <- log.odds.to.prob(mods.base.correct[[3]]$mod$coefficients[1], mods.base.correct[[1]]$mod$coefficients[2], 0))
#probability that a black person receives an above-first-quartile sentence 
(p.black.q1 <- log.odds.to.prob(mods.base.correct[[3]]$mod$coefficients[1], mods.base.correct[[1]]$mod$coefficients[2], 1))
...
results.fname <- sprintf(
  'bounds_results_n%s_alpha%s_blackwhite_sentencing_wrong.rds',
  ndraws,
  alpha * 100
) 
...
### Do these results produce different plots than the sentencing bounds code with the correct sentence level categorizations? 
The confidence interval plots look the same whether sentence.quantiles are categorized by the lowest threshold or highest threshold met by each observed sentence. I'm not sure whether this is a result of a coding mistake, or if the classification doesn't make a difference because we model whether df$SENTENCE > threshold. 
