...
Table 2 compares the naive ATE to the $ATE_{M=1}$ when the response functions involve only the pre-treatment covariates, so we leave out information about number of counts of conviction, whether the case went to trial, and the primary conviction offense. Without adjusting for these post-treatment covariates, every value of the naive ATE and the $ATE_{M=1}$ intervals is larger than in Table 1. As shown in Table 3, race is positively correlated with the post-treatment covariates TRIAL and NOCOUNTS. Because PRIM_OFFENSE is a categorical variable, I defined an indicator of whether a charge would be considered violent. This indicator not based on a statutory definition, and is rather a rough attempt to evaluate the correlation of the post-treatment covariates with race. Table 3 shows that the prevalence of these more serious convictions does differ significantly by the perceived race of the defendant. To be clear: these tables do *not* suggest that there is an intrinsic difference between the behavior of white and Black people. Rather, the correlation between race and conviction severity reflects the impact of systemic racism prior to and during the conviction stage of a case, which is not captured by the mediating variable. When adjusting for post-treatment covariates that are positively correlated with the treatment variable and the response, the estimated treatment effect is smaller relative to the model without these covariates. 
...
These plots show the bounds and confidence intervals for $ATE_{M=1}$ and $ATT_{M=1}$ for a sequence of possible values of $\rho$, represented by the x-axis. The three different plots consider the response variable as whether the sentence length exceeds the third quartile, median, or first quartile sentence length, respectively. Note that while Tables 1 through 4 show the treatment effect as a proportion of individuals who received discriminatory sentences, the y-axis in the plots is scaled by the number of cases in the sample. So in this case, $ATE_{M=1}$ refers to the number of cases that would have had different outcomes if the defendant was of a different race. The opaque red shape represents the bounds for $ATE_{M=1}$. The light red shading covers the confidence interval for the $ATE_{M=1}$ bounds. We see that the confidence intervals are very near the bounds, which is what we expect with a large sample size. The dashed line represents the point estimates for $ATT_{M=1}$. $ATT_{M=1}$ is scaled by the number of cases involving Black people, while $ATE_{M=1}$ is scaled by the total number of cases. 

The shape and size of the bounds differs depending on where we set the sentence length threshold for the response variable. For each threshold, the bounds for higher values of $\rho$ are wider. This increase in bound width is also seen in Mummolo et al's plots. *intuitive explanation?* For large values of $\rho$, there appears to be greater uncertainty about the correct $ATE_{M=1}$ for higher sentence length thresholds. But for smaller values of $\rho$, including Mummolo et al's conservative estimate of $\rho = 0.32$, the bounds for the highest sentence length threshold are the narrowest. Specifically, using the model with only pretreatment covariates, the number of cases affected by discriminatory sentences above 77 months is estimated to be between 16,184 and 29,679 people; for sentences longer than 37 months, somewhere between 22,634 and 48,035 are estimated as discriminatory; and for sentences over 15 months, between 20,958 and 44,764 are labeled discriminatory. I think that the smaller treatment effect estimate for longer sentences may be related to the aforementioned correlation between measures of conviction severity and perceived race. Again, this emphasizes how difficult it is to control for the impact of systemic racism throughout the criminal legal process. Overall, I am not sure how to explain why changing the coding of the response variable changes the magnitude and range of the estimated $ATE_{M=1}$ in the way we observe.   

One general observation is that the scaled $ATT_{M=1}$ is less than the lower bound of the scaled $ATE_{M=1}$. As stated above, the mechanical reason for this is, broadly, that $ATE_{M=1}$ is scaled by the total sample population while $ATT_{M=1}$ is scaled by the number of Black people in the sample. The motivation for this is that $ATT_{M=1}$ quantifies the number of cases involving a Black person that would not have resulted in a sentence above the threshold if the defendant were white, while $ATE_{M=1}$ also counts the number of cases involving a white person that would have resulted in a conviction if they were Black. This is also why it's possible for $ATE_{M=1}$ to exceed the proportion of Black people in the sample (about 0.2735), as we observe in the estimated ATE bounds. 

Specifically, the lower bound of $ATE_{M=1} > P(D_i=1|M_i=1)$ when 
\[
\begin{align*}
...
For the immigration data, there is barely any difference between the bounds computed using the model with the post-treatment TRIAL and NOCOUNTS covariates, versus the bounds using the model without these variables. This seems like a good indicator that the immigration cases are indeed more homogenous than the full set of cases. It seems more plausible, with the immigration data, to assert that race is randomly assigned conditional on pre-treatment covariates. Unfortunately, although Mummolo et al's assumptions seem a bit more reasonable for the immigration data, the bounds on $ATE_{M=1}$ aren't very informative. Particularly for the 1st quartile sentence threshold, the bounds and confidence intervals are very wide, in places dipping below 0, which is clearly incorrect. As stated above, the large value of $P(D_i=0|M_i=1)$ contributes to this particularly low bound. 

Comparing the results for all 2006-2008 cases to the immigration cases, the immigration data is more homogenous, which we see in the fact that controlling for post-treatment covariates changes the results very little. Nonetheless, comparing Tables 2 and 4, the bounds for the immigration $ATE_{M=1}$ are ultimately more uncertain, even when modeling the 2006-08 data with only pre-treatment covariates. 

*other limitations/questions for future research?*
