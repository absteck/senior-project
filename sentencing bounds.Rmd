...
p.white.q1 <- log.odds.to.prob(mods.base.correct[[1]]$mod$coefficients[1], mods.base.correct[[1]]$mod$coefficients[2], 0)
#probability that a black person receives an above-third-quartile sentence 
p.black.q1 <- log.odds.to.prob(mods.base.correct[[1]]$mod$coefficients[1], mods.base.correct[[1]]$mod$coefficients[2], 1)
...
p.white.q3 <- log.odds.to.prob(mods.base.correct[[3]]$mod$coefficients[1], mods.base.correct[[1]]$mod$coefficients[2], 0)
#probability that a black person receives an above-first-quartile sentence 
p.black.q3 <- log.odds.to.prob(mods.base.correct[[3]]$mod$coefficients[1], mods.base.correct[[1]]$mod$coefficients[2], 1)
```
...
if (!file.exists(results.fname)){
  results <- ddply( # https://www.rdocumentation.org/packages/plyr/versions/1.8.6/topics/ddply 
    ...
    function(combo){
      ...
      if (mod.type == 'full'){
        D0.prob <-
          1 / (1 + exp(-(as.numeric(X.full.race %*% coef(mod.race.full))))) #NOTE: need to create full specification model 
        print("using logit transformation")
      } else {
        D0.prob <- mean(df$race == 'white')
        print("using D0.prob = mean(df$race == 'white')")
      } #PROBLEM: neither of these statements are printing, what's going on? 
...
dim(results) # 100 values of rho x 3 sentence thresholds x 2 estimands = 600 rows 
results[201:221, ] 
```
