...
### model formulas ###
#pretreatment <- c('race', 'AGE', 'MALE', 'HSGED', 'SOMEPOSTHS', 'POSTHSDEGREE', 'HISPANIC', 'USCITIZEN', 'SWB', 'CRIMINAL', 'CATEGORY2', 'CATEGORY3', 'CATEGORY4', 'CATEGORY5', 'CATEGORY6', 'POINTS', 'YR2007', 'YR2006') 
formula.pretreat <- formula(
  ~ race + AGE + MALE + HSGED + SOMEPOSTHS + POSTHSDEGREE + HISPANIC + USCITIZEN + SWB + CRIMINAL + CATEGORY2 + CATEGORY3 + CATEGORY4 + CATEGORY5 + CATEGORY6 + POINTS + YR2007 + YR2006
)

formula.pretreat.race <- formula(
  ~ AGE + MALE + HSGED + SOMEPOSTHS + POSTHSDEGREE + HISPANIC + USCITIZEN + SWB + CRIMINAL + CATEGORY2 + CATEGORY3 + CATEGORY4 + CATEGORY5 + CATEGORY6 + POINTS + YR2007 + YR2006
)
...
X.pretreat <- sparse.model.matrix(formula.pretreat, df) 
X.pretreat.race <- sparse.model.matrix(formula.pretreat.race, df)
...
X.pretreat.setDwhite <- sparse.model.matrix(formula.pretreat, 
                                        d.setDwhite, 
...
X.pretreat.setDblack <- sparse.model.matrix(formula.pretreat,
                                        d.setDblack,
                                        drop.unused.levels = FALSE)
...
Xblack.pretreat.setDwhite <- X.pretreat.setDwhite[df$race == 'black',]
Xblack.pretreat.setDblack <- X.pretreat.setDblack[df$race == 'black',]
...
if (!file.exists('mods_pretreat_correct_blackwhite_sentencing.rds')){
  mods.pretreat.correct <- lapply(
    ...
    function(thresh_index){
      cat(rep('=', 80), '\npretreat model, threshold = ', thresh_index,
  ...
  saveRDS(mods.pretreat.correct, 'mods_pretreat_correct_blackwhite_sentencing.rds')
} else {
  mods.pretreat.correct <- readRDS('mods_pretreat_correct_blackwhite_sentencing.rds')
}
...
if (!file.exists(results.fname)){
  results <- ddply( # https://www.rdocumentation.org/packages/plyr/versions/1.8.6/topics/ddply 
    ...
    function(combo){
      ...
      if (!mod.type %in% c('pretreat', 'full')){
      ...
      ates.chunks <- llply(
        ...
        function(chunk.start){
          ...
          pred.setD1.chunk <- logit.sims.predict(
            get(sprintf('X.%s.setD%s', mod.type, treat.race)), #X.pretreat.setDblack: data vectors where treatment is set to 1 
