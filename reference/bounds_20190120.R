...
logit.sims.predict.chunked <- function(X, beta.sims, chunk.size = csize){
...
logit.sims.predictmean.chunked <- function(X, beta.sims, chunk.size = csize){
  if (ncol(beta.sims) %% chunk.size != 0){
    stop('# simulated coefficient draws is not evenly divisible by chunk size')
  }
  out <- llply(
...
formula.full <- formula(
  ~ race + gender + age2 + age2sq + daytime + inout2 + ac_incid * ac_time +
    offunif2 + typeofid2 + othpers2 + cs_objcs + cs_descr + cs_casng +
    cs_lkout + cs_cloth + cs_drgtr + cs_furtv + cs_vcrim + cs_bulge +
    cs_other + pct + year
)

## for race models # QUESTION: what is this race model for? When do we want to predict race? 
## formula.base.race <- formula(~ 1)  # this just gives raw proportions
formula.full.race <- formula(
  ~ gender + age2 + age2sq + daytime + inout2 + ac_incid * ac_time +
    offunif2 + typeofid2 + othpers2 + cs_objcs + cs_descr + cs_casng +
    cs_lkout + cs_cloth + cs_drgtr + cs_furtv + cs_vcrim + cs_bulge +
    cs_other + pct + year
)
...
X.full <- sparse.model.matrix(formula.full, d)
## X.base.race <- sparse.model.matrix(formula.base.race, d)
X.full.race <- sparse.model.matrix(formula.full.race, d)
...
d.setDhispanic <- d; d.setDhispanic$race <- factor('hispanic', levels = races)
...
## set race to hispanic for all encounters
X.base.setDhispanic <- sparse.model.matrix(formula.base,
                                           d.setDhispanic[1, , drop = FALSE],
                                           drop.unused.levels = FALSE
                                           )
X.full.setDhispanic <- sparse.model.matrix(formula.full,
                                           d.setDhispanic,
                                           drop.unused.levels = FALSE
                                           )
...
Xblack.full.setDwhite <- X.full.setDwhite[d$race == 'black',]
Xblack.full.setDblack <- X.full.setDblack[d$race == 'black',]

## avg effect of hispanic (vs white) in hispanic encounters
Xhispanic.base.setDwhite <- X.base.setDwhite
Xhispanic.base.setDhispanic <- X.base.setDhispanic
Xhispanic.full.setDwhite <- X.full.setDwhite[d$race == 'hispanic',]
Xhispanic.full.setDhispanic <- X.full.setDhispanic[d$race == 'hispanic',]



### cleanup ###
rm(d.setDwhite, d.setDblack, d.setDhispanic)



## ################################################################################
## ## verify speedglm coefficient estimates and clustered se against regular glm ##
## ################################################################################

## ## test speedglm analysis...
## start.time.speedglm <- Sys.time()
## mod.base <- speedglm.wfit(y = d$force.binary,
##                           X = X.base,
##                           family = binomial(link = 'logit'),
##                           sparse = TRUE,
##                           trace = TRUE
##                           )
## end.time.speedglm <- Sys.time()
## end.time.speedglm - start.time.speedglm
## vcovCL.base <-
##   vcovCL.speedglm(mod.base, d$force.binary, X.base, cluster = d$pct)

## ## ... against regular glm
## start.time.glm <- Sys.time()
## mod.base.glm <- glm(force.binary ~ race,
##                     family = binomial(link = 'logit'),
##                     data = d
##                     )
## end.time.glm <- Sys.time()
## end.time.glm - start.time.glm
## vcovCL.base.glm <- vcovCL(mod.base.glm, d$pct)

## ## results are identical at least up to 8th decimal place
## coef(mod.base) - coef(mod.base.glm)
## vcovCL.base - vcovCL.base.glm

## rm(mod.base, mod.base.glm, vcovCL.base, vcovCL.base.glm)



#############################################################
## run incorrect models with fryer data-dropping procedure ##
#############################################################

## predicted use of varying force against various racial groups, no covariates
if (!file.exists('intermediate_files/mods_base_fryer.rds')){
  mods.base.fryer <- lapply(
    1:7,  # violence thresholds, e.g. 1 = 'at least use of hands'
    function(thresh){
      cat(rep('=', 80), '\nbase model, threshold = ', thresh,
          ' (', force.levels[thresh], ')\n\n', sep = '')
      fryer.ind <- which(d$force == 0 | d$force >= thresh)
      mod <- speedglm.wfit(y = d$force[fryer.ind] >= thresh,
                           X = X.base[fryer.ind,],
                           family = binomial(link = 'logit'),
                           sparse = TRUE,
                           trace = TRUE
      ...
      vcovCL <- vcovCL.speedglm(mod,
                                d$force[fryer.ind] >= thresh,
                                X.base[fryer.ind,],
                                cluster = d$pct[fryer.ind]
                                )
      return(list(thresh = force.levels[thresh], mod = mod, vcovCL = vcovCL))
    })
  saveRDS(mods.base.fryer, 'intermediate_files/mods_base_fryer.rds')
} else {
  mods.base.fryer <- readRDS('intermediate_files/mods_base_fryer.rds')
}

## predicted use of varying force against various racial groups, with covariates
if (!file.exists('intermediate_files/mods_full_fryer.rds')){
  mods.full.fryer <- lapply(
    1:7,  # violence thresholds, e.g. 1 = 'at least use of hands'
    function(thresh){
      cat(rep('=', 80), '\nfull model, threshold = ', thresh,
          ' (', force.levels[thresh], ')\n\n', sep = '')
      fryer.ind <- which(d$force == 0 | d$force >= thresh)
      mod <- speedglm.wfit(y = d$force[fryer.ind] >= thresh,
                           X = X.full[fryer.ind,],
  ...
  saveRDS(mods.full.fryer, 'intermediate_files/mods_full_fryer.rds')
} else {
  mods.full.fryer <- readRDS('intermediate_files/mods_full_fryer.rds')
}
...
## predicted use of varying force against various racial groups, no covariates
if (!file.exists('intermediate_files/mods_base_correct.rds')){
...
saveRDS(mods.base.correct, 'intermediate_files/mods_base_correct.rds')
} else {
  mods.base.correct <- readRDS('intermediate_files/mods_base_correct.rds')
}

## predicted use of varying force against various racial groups, with covariates
if (!file.exists('intermediate_files/mods_full_correct.rds')){
  ...
  saveRDS(mods.full.correct, 'intermediate_files/mods_full_correct.rds')
} else {
  mods.full.correct <- readRDS('intermediate_files/mods_full_correct.rds')
}

## race (white vs not white) of civilian in encounter, no covariates
mean(d$race == 'white')
## predicted race (white vs not white) of civilian in encounter, with covariates
mod.race.full <- speedglm.wfit(y = d$race == 'white',
                               X = X.full.race,
                               family = binomial(link = 'logit'),
                               sparse = TRUE,
                               trace = TRUE
...
rhos <- seq(.01, .99, .01)
combos <- expand.grid(treat.race = c('black', 'hispanic'),
                      mod.type = c('base', 'full'),
                      thresh = 1:7,
                      stringsAsFactors = FALSE
                      )

## clean up failed jobs from parallelization
combos <- data.table::fread('intermediate_files/tmp.csv')
results.fname <- sprintf('intermediate_files/bounds_results_n%s_alpha%s_cleanup.rds',
                         ndraws,
                         alpha * 100
                         )
