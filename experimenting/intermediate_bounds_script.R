...
df <- df %>% mutate ( # https://dplyr.tidyverse.org/reference/mutate.html
  sentence.quartiles = case_when( # https://dplyr.tidyverse.org/reference/case_when.html 
    ...
    SENTENCE > 0.03 ~ 'above 0.03' #TEMPORARY
  ))

thresholds <- c(72, 37, 15, 0.03) 
...
#intermediate specification, to test that model with covariates works
...
#442187 rows, 2 cols. same nrows as df
...
(p.white.q3 <- log.odds.to.prob(mods.base.correct[[1]]$mod$coefficients[1], mods.base.correct[[1]]$mod$coefficients[2], 0))
#probability that a black person receives an above-third-quartile sentence 
(p.black.q3 <- log.odds.to.prob(mods.base.correct[[1]]$mod$coefficients[1], mods.base.correct[[1]]$mod$coefficients[2], 1))
...
fake_x_q2_black.murder <-c(1, mean(df$MALE[sentence.quartiles %in% c("above median")]))
(p.black.full.q2.murder <- log.odds.to.prob(mods.full.correct[[2]]$mod$coefficients[1], mods.full.correct[[1]]$mod$coefficients[2:3], fake_x_q2_black.murder))

fake_x_q2_white.murder <-c(0, mean(df$MALE[sentence.quartiles %in% c("above median")]))
(p.white.full.q2.murder <- log.odds.to.prob(mods.full.correct[[2]]$mod$coefficients[1], mods.full.correct[[1]]$mod$coefficients[2:3], fake_x_q2_white.murder)) #0.946
...
#ndraws <- 5000
ndraws <- 500 #TEMPORARY: reduced ndraws to simplify 
alpha <- .95
chunk.size <- 250 
...
#rhos <- seq(.01, .99, .01) 
rhos <- c(0.5, 0.99) #TEMPORARY: switch interval back to 0.01

combos <- expand.grid(treat.race = 'black',
                      mod.type = c('base', 'full'), #NOTE: ultimately want to add 'full' 
                      thresh = 1, #TEMPORARY: only using thresh = 1 to hopefully speed computation 
